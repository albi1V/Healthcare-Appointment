...
interface Slot {
  ...
  isoForControl: string; // yyyy-MM-ddTHH:mm (local)
  disabled: boolean;
  taken?: boolean;
}
...
export class SymptomRecommendationComponent {
  ...
  constructor(
    ...
    this.itemForm = this.fb.group({
      ...
      time: ['', Validators.required] // stores local ISO yyyy-MM-ddTHH:mm
    });
  ...
  /* ================= UTIL: styled console logs ================= */
 
  /** Red badge log for success/error highlight */
      ...
      next: (appts: any[]) => {
        ...
        for (const a of appts || []) {
          ...
          if (raw.startsWith(this.selectedDate)) {
            // We compare at minute precision
            this.takenSlotKeys.add(raw.slice(0, 16)); // yyyy-MM-ddTHH:mm
          }
        }
